...
survey2 <- survey1 %>% dplyr::select(-Sodium_Intake) %>% na.omit()
...
# Performed on non-imputed dataset
logit1 <- glm(Heart_Disease ~ as.factor(Depression), data = survey1)
summary(logit1)
# Individuals that have been told they have a depressive disorder have a 0.028 higher log odds of ever having reported coronary heart disease or myocardial infarction compared to individuals that have never been told they have a depressive disorder
...
# Performed on non-imputed dataset
logit2 <- glm(Heart_Disease ~ Ment_Health, data = survey1)
summary(logit2)
...
# Predict outcome in test set
predicted1 = step.model %>% predict(test.set.x, type = "response")
predicted.classes1 = ifelse(predicted1 > 0.5, 1, 0)

# Creating cross table
cross1 <- CrossTable(test.set.y$Heart_Disease, predicted.classes1, prop.chisq = FALSE)

# Calculating Precision
precision1 <- cross1$prop.col[4]
precision1

# Calculating Recall
recall1 <- cross1$prop.row[4]
recall1

# Calculating F1 Score
F1_step <- (2 * (precision1 * recall1)) / (precision1 + recall1)
F1_step
```